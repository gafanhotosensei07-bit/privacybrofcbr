...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (table === "profiles") {
      ({ data, error } = await supabase.from("profiles").select("*").order("created_at", { ascending: false }).limit(500));
    } else if (table === "conversations") {
      ({ data, error } = await supabase.from("conversations").select("*").order("created_at", { ascending: false }).limit(500));
      ...
      ({ data, error } = await supabase.from("checkout_attempts").select("*").order("created_at", { ascending: false }).limit(500));
      ...
      if (!file || !filePath) return new Response(JSON.stringify({ error: "file e file_path são obrigatórios" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const { data: uploadData, error: uploadError } = await supabase.storage.from("model-content").upload(filePath, file, { upsert: true });
      if (uploadError) throw uploadError;
      const { data: urlData } = supabase.storage.from("model-content").getPublicUrl(filePath);
      data = { path: uploadData?.path, publicUrl: urlData?.publicUrl };
      error = null;
    } else if (table === "list_content") {
      const folder = body.folder || "";
      const { data: files, error: listError } = await supabase.storage.from("model-content").list(folder, { limit: 200, sortBy: { column: "created_at", order: "desc" } });
      ...
      if (!file_paths || !Array.isArray(file_paths)) return new Response(JSON.stringify({ error: "file_paths array é obrigatório" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const { error: delError } = await supabase.storage.from("model-content").remove(file_paths);
      if (delError) throw delError;
      data = { deleted: file_paths.length };
      error = null;
    } else if (table === "page_views") {
      ({ data, error } = await supabase.from("page_views").select("*").order("created_at", { ascending: false }).limit(2000));
      ...
      pageViews.forEach((pv: any) => {
        ...
        if (!bySource[src]) bySource[src] = { clicks: 0, checkouts: 0, approved: 0, revenue: 0 }; bySource[src].clicks++;
      ...
      const checkouts = checkoutsRes.data || []; const pageViews = pageViewsRes.data || [];
      const approved = checkouts.filter((c: any) => c.payment_status === "approved");
      const revenue = approved.reduce((sum: number, c: any) => sum + Number(c.plan_price || 0), 0);
      const modelViewCounts: Record<string, number> = {}; const modelCheckoutCounts: Record<string, number> = {}; const pageTypeCounts: Record<string, number> = {};
      pageViews.filter((pv: any) => pv.page_type === "modelo").forEach((pv: any) => { modelViewCounts[pv.page_slug] = (modelViewCounts[pv.page_slug] || 0) + 1; });
      checkouts.forEach((c: any) => { modelCheckoutCounts[c.model_name] = (modelCheckoutCounts[c.model_name] || 0) + 1; });
      pageViews.forEach((pv: any) => { pageTypeCounts[pv.page_type] = (pageTypeCounts[pv.page_type] || 0) + 1; });
      const now = new Date(); const last24h = new Date(now.getTime() - 86400000);
      data = {
        totalUsers: profilesRes.count || 0, totalConversations: conversationsRes.count || 0,
        totalCheckouts: checkouts.length, approvedCheckouts: approved.length, totalRevenue: revenue,
        totalPageViews: pageViews.length,
        recentViews24h: pageViews.filter((pv: any) => new Date(pv.created_at) > last24h).length,
        recentCheckouts24h: checkouts.filter((c: any) => new Date(c.created_at) > last24h).length,
        modelViews: modelViewCounts, modelCheckouts: modelCheckoutCounts, pageTypeBreakdown: pageTypeCounts,
        conversionRate: pageViews.length > 0 ? ((checkouts.length / pageViews.length) * 100).toFixed(1) : "0",
      };
      error = null;
    } else if (table === "members") {
      const { data: approvedCheckouts, error: membersError } = await supabase.from("checkout_attempts").select("*").eq("payment_status", "approved").order("created_at", { ascending: false }).limit(1000);
      if (membersError) throw membersError;
      const userIds = [...new Set((approvedCheckouts || []).map((c: any) => c.user_id).filter(Boolean))];
      let profilesMap: Record<string, string> = {};
      if (userIds.length > 0) { const { data: profs } = await supabase.from("profiles").select("user_id, display_name").in("user_id", userIds); (profs || []).forEach((p: any) => { profilesMap[p.user_id] = p.display_name; }); }
      data = (approvedCheckouts || []).map((c: any) => ({ ...c, display_name: profilesMap[c.user_id] || c.customer_name || "—" }));
      error = null;
    } else if (table === "revoke_access") {
      const { checkout_id } = body;
      if (!checkout_id) return new Response(JSON.stringify({ error: "checkout_id é obrigatório" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      ({ data, error } = await supabase.from("checkout_attempts").update({ payment_status: "rejected" }).eq("id", checkout_id).select());
    } else if (table === "domains") {
      ({ data, error } = await supabase.from("monitored_domains").select("*").order("created_at", { ascending: false }));
    } else if (table === "add_domain") {
      const { domain, label, notes } = body;
      if (!domain) return new Response(JSON.stringify({ error: "domain é obrigatório" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      ({ data, error } = await supabase.from("monitored_domains").insert({ domain, label: label || "", notes: notes || "" }).select());
      ...
      const updates: any = {};
      if (domain !== undefined) updates.domain = domain; if (label !== undefined) updates.label = label; if (notes !== undefined) updates.notes = notes; if (is_active !== undefined) updates.is_active = is_active;
      ({ data, error } = await supabase.from("monitored_domains").update(updates).eq("id", domain_id).select());
